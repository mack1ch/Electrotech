...
export function ProductDetailBreadcrumbs({ productName }: { productName: string }) {
  return (
    <nav aria-label="Хлебные крошки">
      <Breadcrumb
        ...
        items={[
          ...
          {
            title: (
              <Link href="/search" className="font-normal text-[#4a5565] hover:underline">
                Товары
              </Link>
            ),
          },
          { title: <span className="font-normal text-brand">{productName}</span> },
        ]}
      />
    </nav>
  );
